...
$.fn.setResults = function(data, options) {
	...
	for (i in data) {
		...
		t += '<td class="track_button_cell"><a href="javascript:player.playTrack(${trackId}, ${albumId})" class="track_button ${buttonStyle}"></a></td><td class="track_price"></td><div>${price}</div><td class="track_genres"><div>${genres}</div></td><td class="album_name"><div>${albumName}</div></td>';
		t += '<td class="track_artists"><div>${artists}</div></td>';
		t += (options && options['playlist']) ? '<td class="track_delete"><a href="#" class="pl_remove" title="Delete From Playlist">X</a></td>' : '';
		t += '</tr>';
